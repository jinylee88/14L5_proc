function [u cc Iup Qup] = runLoupas(I, Q, interpFactor, kernelLength, axial, par)
...
tstart = tic;
u = zeros(size(Iup));
fprintf(1, 'Displacement Estimation...');
    tmp = zeros(size(Iup,1),size(Iup,2)*size(Iup,3));
   
    parfor ii = 1:size(Iup,2)*size(Iup,3)
        k = mod(ii-1,size(Iup,3))+1;
        i = ceil(ii/size(Iup,3));
        tmp(:,ii) = loupas(squeeze(Iup(:,i,par.nref)),squeeze(Qup(:,i,par.nref)),squeeze(Iup(:,i,k)),squeeze(Qup(:,i,k)),size(Iup,1),kasai_avg,fdem_vec,fc_vec,kasai_scale);
    end
    
fprintf('\n');
tend = toc(tstart);
u = reshape(tmp,size(Iup,1),size(Iup,3),size(Iup,2));
u = permute(u,[1 3 2]);
u = -u.*1e6;
