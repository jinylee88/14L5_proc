function Resfiles = procArfi_Realtime(varargin)
tic
...
lastprocessed = '';
dirname = fileparts(filenames);
fname = '';
fileidx = 1;
while 1
    while strcmpi(fname,lastprocessed);
    fNames = dir(fullfile(dirname,'SWIF_AData*.bin'));
    if length(fNames)>=fileidx;
    fname = fNames(fileidx).name;
    end
    end
...
%     end
    ...
    [data swifParams0]= readSwif(fname, dimsname);
    ...
    if strcmp(swifParams.imagingMode,'bmode')
        ...
        if ~isfield(par,'pushbeamNum')
            par.pushbeamNum = 1:par.numBeamGroups;
        end
        ...
        for frmii = 1:length(Frameindices)
            ...
            switch lower(swifParams.imagingModeShort)
                ...
                case 'arfi'
                    ...
                    Cidx1 = Cidx;
                case 'arfiswei'
                    cc_onax = squeeze(mean(cc_coef(:,16:17,:,:),2));
                    sweidata0 = arfidata0;
                    arfidata0 = squeeze(mean(arfidata0(:,16:17,:,:),2));
                    sweidata = arfidata;
                    arfidata = squeeze(mean(arfidata(:,16:17,:,:),2));
                    bmwidth = 2 * 1e3 * (par.c/(par.F0MHz*1e6))' * par.pushFnum;
            ...
            if options.save.Frames
                ...
                if exist('sweidata','var')
                    ...
                    sweidata0 = int16(sweidata/arfi_scale);
                    
                end
                switch par.probe        
                    case 'linear'
                        save(resfile(frmidx,:),'arfidata','sweidata','cc','axial','lat','t','bmodedata0','blat','arfidata0','arfi_scale','ccmap','iq0')
                    case 'phased'
                        if exist('sweidata','var')
