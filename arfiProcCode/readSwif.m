...
[filePath, fname] = fileparts(fname);
fname = strcat(fname, '.bin');
if nargin<2
    dimsname = fullfile(filePath,sprintf('SWIF_ADataDims_%s.txt', fname(12:end-4)));
    if ~exist(dimsname, 'file'),dimsname = dir('SWIF_ADataDims_*.txt');dimsname = dimsname(end).name;end
...
fid = fopen(fullfile(filePath,fname), 'rb');
