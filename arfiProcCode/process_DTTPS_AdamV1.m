function [dttps_file] = process_DTTPS_AdamV1(datapath,useparfor,DSratio,nBeamsFullySampled)
...
if nargin<4
    nBeamsFullySampled=32;
end
TrackBeams=nBeamsFullySampled;
...
for ind = 1:length(swifFiles)
        timeStamp = swifFiles(ind).name(12:end-4);
        resfile = ['resTrack' num2str(TrackBeams) 'DS' num2str(DSratio) '_' timeStamp '.mat'];
        parfile = ['parTrack' num2str(TrackBeams) 'DS' num2str(DSratio) '_' timeStamp '.mat'];
...
%resFiles = dir(fullfile(datapath,'resDSx2_*.mat'));
resFiles = dir(fullfile(datapath,['resTrack' num2str(TrackBeams) 'DS' num2str(DSratio) '_*.mat']));
resFiltFiles = dir(fullfile(datapath,'resFilt_*.mat'));
parFiles = dir(fullfile(datapath,['parTrack' num2str(TrackBeams) 'DS' num2str(DSratio) '_*.mat']));
dttps_file = fullfile(datapath,['dttpsTrack' num2str(TrackBeams) 'DS' num2str(DSratio) '.mat']);
...
for resIdx = 1:numSWEI
    ...
    data.I=reshape(data.I,144,32,42,42);
    data.I=data.I(:,1:DSratio:nBeamsFullySampled,1:DSratio:end,:);
    data.I=reshape(data.I,144,[],42);
    
    data.Q=reshape(data.Q,144,32,42,42);
    data.Q=data.Q(:,1:DSratio:nBeamsFullySampled,1:DSratio:end,:);
    data.Q=reshape(data.Q,144,[],42);
...
v = zeros(size(sweidata,1),size(sweidata,2),size(sweidata,3)-4,'single');
arfidata_tmp = sweidata(:,:,4:end);
t_tmp = t(4:end);
...
tpkLR = nanmean(cat(4,tpkR,tpkL),4);
tpkLR1 = nanmean(cat(4,tpkR1,tpkL1),4);
...
tpkRclip = tpkR(:,(bmIdxFull(1,1):bmIdxFull(nPush/2,1))-bmPattern(1,1),:);
...
dlat =  nanmean(cat(4,dxLclip,dxRclip),4);
...
%%
ttps = nanmean(cat(4,tpkLclip,tpkRclip),4);
ttps1 = nanmean(cat(4,tpkL1clip,tpkR1clip),4);
...
save(dttps_file,'ttps','ttps1','dlat','dttpsRmtl','dttpsLmtl','dttpsRstl','dttpsLstl','arfidata','x','z','x1','z1')
fprintf('done\n');
